...
$$
입력 시퀀스 = x_1,x_2,x_3,...,x_n \\ h_i = W_h⋅x_i \\ \\
Q_i = W_Q⋅h_i \\ K_i = W_k⋅h_i \\ V_i=W_v⋅h_i
$$

$$
score_{1i} = Q_1K_i \\ 
α_{1i} = Softmax(score_{1i}) \\
z_1 = \sum α_{1i} V_i \\
output = W_o ⋅ z_1
$$
...
- ##### 멀티 헤드 어텐션(Multi-head Attention)
  ...
  **어텐션 (Attention)**: 일반적인 어텐션 메커니즘은 두 개의 다른 시퀀스 사이의 연관성을 계산함.

  **셀프 어텐션 (Self-Attention)**: 셀프 어텐션은 **동일한 시퀀스 내**에서 각 요소가 **다른 모든 요소들**과의 연관성을 계산함

<img src="../images/2024-08-16-week_2/image-20240820101649949.png" alt="image-20240820101649949" style="zoom: 67%;" /><img src="https://gaussian37.github.io/assets/img/dl/concept/transformer/0.png" alt="Transformer 모델 (Attention is all you need) - gaussian37" style="zoom:50%;" />
